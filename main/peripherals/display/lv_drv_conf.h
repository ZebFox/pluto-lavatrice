 ...
 * DELAY INTERFACE
 *********************/
#define LV_DRV_DELAY_INCLUDE  "utils/utils.h"            /*Dummy include by default*/
 ...
 *  Common
 *------------*/
#define LV_DRV_DISP_INCLUDE         "peripherals/display/display_common.h"           /*Dummy include by default*/
#define lv_drv_set_data_command(val)  set_data_command(val)    /*Set the command/data pin to 'val'*/
#define lv_drv_set_reset(val)       set_reset(val)    /*Set the reset pin to 'val'*/

/*---------
 *  SPI
 *---------*/
#define LV_DRV_DISP_SPI_CS(val)          /*Set the SPI's Chip select to 'val'*/
#define LV_DRV_DISP_SPI_WR_BYTE(data)    disp_spi_send_data(&data, 1)        /*Write a byte the SPI bus*/
#define LV_DRV_DISP_SPI_WR_ARRAY(adr, n) disp_spi_send_data(data, n)  /*Write 'n' bytes to SPI bus from 'adr'*/
 ...
 *  Common
 *----------*/
#define LV_DRV_INDEV_INCLUDE     "peripherals/tsc2046.h"             /*Dummy include by default*/
 ...
 *  SPI
 *---------*/
#define LV_DRV_INDEV_SPI_CS(val)            tp_spi_cs(val)/*spi_cs_set(val)*/     /*Set the SPI's Chip select to 'val'*/
#define LV_DRV_INDEV_SPI_XCHG_BYTE(data)    tp_spi_exchange_byte(data, 1) /*spi_xchg(val)*/     /*Write 'val' to SPI and give the read value*/
