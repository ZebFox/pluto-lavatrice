...
struct page_data {
    ...
    uint8_t da_value;
    uint8_t run;
    uint8_t original_scale;
};


static const char *TAG = "PageTestMems";
...
static void update_page(model_t *pmodel, struct page_data *pdata) {
    const uint16_t gs[6]     = {2, 4, 8, 2, 4, 8};
    const uint16_t offset[6] = {1, 1, 1, 0, 0, 0};
    uint16_t       scala     = pmodel->prog.parmac.scala_accelerometro;

    lv_label_set_text_fmt(pdata->lbl_axes, "x=%03i y=%03i z=%03i", pmodel->test.log_accelerometro[0],
                          pmodel->test.log_accelerometro[1], pmodel->test.log_accelerometro[2]);
    lv_label_set_text_fmt(pdata->lbl_conf, "scala:%i (%ig), OFS=%i", scala, gs[scala], offset[scala]);
    lv_label_set_text_fmt(pdata->lbl_da, "DA=[%04i] %4irpm", pdata->da_value, model_get_velocita_corretta(pmodel));
    lv_label_set_text_fmt(pdata->lbl_signal, "Output signal=%02i.%02iV", DA2RPM(pdata->da_value) / 100,
...
static void *create_page(model_t *pmodel, void *extra) {
    ...
    pdata->original_scale   = pmodel->prog.parmac.scala_accelerometro;
    return pdata;
}
...
static void open_page(model_t *pmodel, void *args) {
    ...
    view_common_title(lv_scr_act(), "TEST MEMS");
    ...
    lv_obj_set_style(lbl, &style_label_6x8);
    lv_obj_set_auto_realign(lbl, 1);
    lv_obj_align(lbl, NULL, LV_ALIGN_IN_TOP_LEFT, 4, 23);
    pdata->lbl_conf = lbl;

    lbl = lv_label_create(lv_scr_act(), NULL);
    lv_obj_set_style(lbl, &style_label_6x8);
    lv_obj_set_auto_realign(lbl, 1);
    lv_obj_align(lbl, NULL, LV_ALIGN_IN_TOP_LEFT, 4, 31);
    pdata->lbl_da = lbl;

    lbl = lv_label_create(lv_scr_act(), NULL);
    lv_obj_set_style(lbl, &style_label_6x8);
    lv_obj_set_auto_realign(lbl, 1);
    lv_obj_align(lbl, NULL, LV_ALIGN_IN_TOP_LEFT, 4, 39);
    pdata->lbl_signal = lbl;
            ...
            if (event.key_event.event == KEY_CLICK) {
                switch (event.key_event.code) {
                    case BUTTON_SINISTRA: {
                        ...
                        pmodel->prog.parmac.scala_accelerometro = pdata->original_scale;
                        msg.cmsg.code                           = VIEW_CONTROLLER_COMMAND_CODE_SEND_PARMAC_AND_TURNOFF;
                        break;
                    }
                    ...
                    case BUTTON_MENU:
                        pmodel->prog.parmac.scala_accelerometro = (pmodel->prog.parmac.scala_accelerometro + 1) % 6;
                        update_page(pmodel, pdata);
                        msg.cmsg.code = VIEW_CONTROLLER_COMMAND_CODE_SEND_PARMAC;
                        break;
...
const pman_page_t page_test_mems = {
    .create        = create_page,
    .open          = open_page,
