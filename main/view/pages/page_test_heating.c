...
static const uint16_t transform[4] = {OUT_SCARICO, OUT_FREDDO, OUT_CALDO, OUT_RESISTENZE};
...
static void update_page(model_t *pmodel, struct page_data *pdata) {
    ...
    if (!test_resistenze_in_sicurezza(pmodel, pdata)) {
        lv_label_set_text(pdata->lbl_res_ok, "no");
    } else {
        lv_label_set_text(pdata->lbl_res_ok, "ok");
    }
}
...
static void open_page(model_t *pmodel, void *args) {
    ...
    view_common_title(lv_scr_act(), view_intl_get_string(pmodel, STRINGS_TEST_RISCALDO));
    ...
    lv_obj_set_style(lbl, &style_label_6x8);
    lv_obj_set_auto_realign(lbl, 1);
    lv_obj_align(lbl, NULL, LV_ALIGN_IN_TOP_RIGHT, -4, 18);
...
static view_message_t process_page_event(model_t *pmodel, void *args, pman_event_t event) {
    ...
    switch (event.code) {
        case VIEW_EVENT_CODE_MODEL_UPDATE:
            ...
            if (((pdata->outputs & (1 << OUT_RESISTENZE)) > 0) && !test_resistenze_in_sicurezza(pmodel, pdata)) {
                pdata->outputs &= ~(1 << OUT_RESISTENZE);
                msg.cmsg.code   = VIEW_CONTROLLER_COMMAND_CODE_TEST_DIGOUT_MULTI;
                msg.cmsg.output = OUT_RESISTENZE;
                msg.cmsg.value  = 0;
            } else if (model_alarm_code(pmodel) > 0 && pdata->outputs > 0) {
...
static int test_resistenze_in_sicurezza(model_t *model, struct page_data *data) {
    return (model->test.inputs & 0x01) && model->run.macchina.temperatura < model->prog.parmac.temperatura_sicurezza &&
           model->run.macchina.temperatura < 50 &&
           model_get_livello_centimetri(model) >= model_get_minimo_livello_riscaldo(model) && model_oblo_chiuso(model);
}
...
const pman_page_t page_test_heating = {
    .create        = create_page,
    .open          = open_page,
