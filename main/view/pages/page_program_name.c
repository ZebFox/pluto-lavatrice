            ...
            if (event.key_event.event == KEY_CLICK) {
                switch (event.key_event.code) {
                    ...
                    case BUTTON_MENU:
                        ...
                        msg.vmsg.page = (void *)&page_program_price;
                        break;
                    ...
                    case BUTTON_LINGUA: {
                        char  *name     = model_get_program(pmodel)->nomi[data->language];
...
static void update_page(model_t *pmodel, struct page_data *pdata) {
    ...
    char  *name     = model_get_program(pmodel)->nomi[pdata->language];
    size_t name_len = strlen(name);
    lv_label_set_text_fmt(pdata->lbl_name, "%s%c", model_get_program(pmodel)->nomi[pdata->language],
                          (pdata->flag && name_len < MAX_NAME_LENGTH) ? '_' : ' ');
