...
static const char *TAG = "PageTestRotation";
...
static void update_page(model_t *pmodel, struct page_data *pdata) {
    lv_label_set_text_fmt(pdata->lbl_da, "DA=[%04i] %4irpm", pdata->da_value, DA2RPM(pdata->da_value));
...
static void open_page(model_t *pmodel, void *args) {
    ...
    if (pdata->forward) {
        view_common_title(lv_scr_act(), view_intl_get_string(pmodel, STRINGS_TEST_MOTORE_AVA));
    } else {
        view_common_title(lv_scr_act(), view_intl_get_string(pmodel, STRINGS_TEST_MOTORE_IND));
    }
            ...
            if (event.key_event.event == KEY_CLICK) {
                switch (event.key_event.code) {
                    ...
                    case BUTTON_START:
                        ...
                        msg.cmsg.output = pdata->forward ? OUT_AVANTI : OUT_INDIETRO;
                ...
                switch (event.key_event.code) {
                    ...
                    case BUTTON_PIU: {
                        pdata->da_value = (pdata->da_value + 1) % 255;
                        update_page(pmodel, pdata);
                        break;
                    }
                }
...
const pman_page_t page_test_rotation = {
    .create        = create_page,
    .open          = open_page,
